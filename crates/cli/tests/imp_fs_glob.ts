...

{
  ...
  console.assert(results.some(r => r.includes("subdir")), "subdir files present");
  console.log("PASS: regression — walk relative paths correct");
}

{
  ...
  const strResults = results.map(r => r.toString());
  console.assert(strResults.length === 2, `expected 2 .txt files, got ${strResults.length}`);
  console.assert(strResults.some(r => r.includes("subdir")), "found file in subdir");
