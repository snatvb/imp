...
const URLS = {
  ...
  long:
    "https://example.com/" + "a".repeat(200) + "?" + Array.from({ length: 50 }, (_, i) => `key${i}=val${i}`).join("&"),
}
...
async function main() {
  ...
  {
    const iters = 100000
    const r = await bench("URL parse simple", iters, () => {
      new URL(URLS.simple)
    })
  ...
  {
    const iters = 100000
    const r = await bench("URL parse complex", iters, () => {
      new URL(URLS.complex)
    })
  ...
  {
    const iters = 50000
    const r = await bench("URL parse long query", iters, () => {
      new URL(URLS.long)
    })
...

  {
    const url = new URL(URLS.complex)
    const iters = 200000
    const r = await bench("URL.toString()", iters, () => {
      url.toString()
    })
...

  {
    const url = new URL(URLS.complex)
    const iters = 200000
    const r = await bench("searchParams.get", iters, () => {
...

  {
    const url = new URL(URLS.complex)
    const iters = 200000
    const r = await bench("searchParams.has", iters, () => {
...

  {
    ...
    const iters = 100000
    const r = await bench("searchParams.toString (50 params)", iters, () => {
