...
const MEDIUM_XML = "<root><users><item><id>1</id><name>Alice</name><email>alice@example.com</email><roles><item>admin</item><item>user</item></roles></item><item><id>2</id><name>Bob</name><email>bob@example.com</email><roles><item>user</item></roles></item><item><id>3</id><name>Charlie</name><email>charlie@example.com</email><roles></roles></item></users><meta><total>3</total><page>1</page><perPage>10</perPage></meta></root>";
...
const MEDIUM_RON = '(users: [(id: 1, name: "Alice", email: "alice@example.com", roles: ["admin", "user"]), (id: 2, name: "Bob", email: "bob@example.com", roles: ["user"]), (id: 3, name: "Charlie", email: "charlie@example.com", roles: [])], meta: (total: 3, page: 1, perPage: 10))';
const LARGE_RON = ron.stringify(LARGE_OBJ).toString();

const SMALL_CSV = "name,value,active\ntest,42,true\n";
const MEDIUM_CSV = "id,name,email,roles\n1,Alice,alice@example.com,admin;user\n2,Bob,bob@example.com,user\n3,Charlie,charlie@example.com,\n";
...
interface BenchResult { avg: number; ops: number; elapsed: number }
interface BenchRow { name: string; parse: BenchResult[]; stringify: BenchResult[] }
...
async function main() {
    ...
    for (const size of ["small", "medium", "large"] as const) {
        ...
        const r1 = await bench(`JSON native parse ${size}`, n, () => { JSON.parse(SMALL_JSON); });
        ...
        const r8 = await bench(`MsgPack parse ${size}`, n, () => { msgpack.parse(msgpackBuf); });
    ...
    for (const size of ["small", "medium", "large"] as const) {
        ...
        const r1 = await bench(`JSON native stringify ${size}`, n, () => { JSON.stringify(obj); });
        ...
        const r4 = await bench(`XML stringify ${size}`, n, () => { xml.stringify(obj, "root"); });
        xmlRow.stringify.push(r4);
        console.log(`  XML:         ${fmt(r4)}`);

        const r5 = await bench(`TOML stringify ${size}`, n, () => { toml.stringify(obj); });
        tomlRow.stringify.push(r5);
        console.log(`  TOML:        ${fmt(r5)}`);

        const r6 = await bench(`RON stringify ${size}`, n, () => { ron.stringify(obj); });
        ronRow.stringify.push(r6);
        console.log(`  RON:         ${fmt(r6)}`);

        const csvObj = size === "small" ? SMALL_CSV_OBJ : size === "medium" ? MEDIUM_CSV_OBJ : LARGE_OBJ.items;
        const r7 = await bench(`CSV stringify ${size}`, n, () => { csv.stringify(csvObj); });
        csvRow.stringify.push(r7);
        console.log(`  CSV:         ${fmt(r7)}`);

        const r8 = await bench(`MsgPack stringify ${size}`, n, () => { msgpack.stringify(obj); });
