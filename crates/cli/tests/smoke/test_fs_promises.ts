...
assert(typeof content === "string", "readFile returns string")
assert(content.includes("this is regular text"), "readFile content correct")
console.log("PASS: fs/promises readFile")

const namedContent = await readFile(import.meta.dirname + "/text.txt", "utf8")
assert(typeof namedContent === "string", "named readFile returns string")
assert(namedContent.includes("this is regular text"), "named readFile content correct")
...
try {
  ...
  threw = true
  assert(String(e).includes("ENOENT") || String(e).includes("not found"), "error message valid")
}
...
assert(written === 12, "writeFile returns bytes written")
const content2 = await readFile(writePath, "utf8")
assert(content2 === "test content", "writeFile content correct")
console.log("PASS: writeFile fs/promises")

const written2 = await writeFile(writePath, " more", "a")
assert(written2 === 5, "named writeFile append returns bytes")
const content3 = await readFile(writePath, "utf8")
assert(content3 === "test content more", "named writeFile append correct")
