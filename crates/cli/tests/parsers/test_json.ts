...

{
  ...
  assert(str === '{"x":1,"y":[1,2,3],"z":{"nested":"value"}}', "stringify should work")
}

{
  const arr = [1, "two", false, null]
  const str = json.stringify(arr).toString()
  assert(str === '[1,"two",false,null]', "array stringify should work")
}
...

{
  ...
  assert(error, "invalid JSON should throw error")
}

{
  ...
  assert(error, "empty string should throw error")
}
...

{
  ...
  assert(error, "unquoted key should throw error")
}

{
  ...
  assert(parsed[0] === 1 && parsed[1] === 2 && parsed[2] === 3, "set values")
}

{
  ...
  const str = json.stringify(set as any).toString()
  const parsed = json.parse(str) as any[]
  assert(Array.isArray(parsed), "set of objects should become array")
  assert(parsed.length === 2, "set length should be 2")
  assert(parsed[0].a === 1, "set first object")
  assert(parsed[1].b === 2, "set second object")
}
...

{
  ...
  const str = json.stringify(map as any).toString()
  const parsed = json.parse(str) as any
  assert(RsString.equals(parsed["1"], "one"), "map numeric key 1")
  assert(RsString.equals(parsed["2"], "two"), "map numeric key 2")
}

{
  const date = new Date("2025-01-01T00:00:00.000Z")
  const str = json.stringify(date as any).toString()
  assert(str === '"2025-01-01T00:00:00.000Z"', "date should become ISO string")
}

{
  const regexp = /hello/gi
  const str = json.stringify(regexp as any).toString()
  assert(str.includes("hello"), "regexp should contain pattern")
}
...

{
  ...
  assert(error, "circular reference should throw error")
}
...

{
  ...
  const parsed = json.parse(input) as any
  assert(parsed.max_i32 === 2147483647, "max i32 should work")
}

{
  const input = '{"over_i32":2147483648}'
  const parsed = json.parse(input) as any
  assert(parsed.over_i32 === 2147483648, "i32+1 should not truncate")
}
...

{
  ...
  const parsed = json.parse(str) as any
  assert(JSON.stringify(parsed) === "{}", "empty object roundtrip")
}

{
  ...
  const parsed = json.parse(str) as any[]
  assert(Array.isArray(parsed) && parsed.length === 0, "empty array roundtrip")
}
