...

{
  ...
  console.assert(r === "aGVsbG8=", `encode "hello" -> ${r}`)
}

{
  const r = base64.encode("hello", { variant: "url", pad: false })
  console.assert(r === "aGVsbG8", `encode url no-pad -> ${r}`)
}

{
  const r = base64.encode("hi", { variant: "url" })
  console.assert(r === "aGk=", `encode url default pad -> ${r}`)
}
...

{
  ...
  console.assert(r === "YQ==", `encode "a" -> ${r}`)
}

{
  const buf = utf8.encode("hello")
  const r = base64.encode(buf)
  console.assert(r === "aGVsbG8=", `encode ByteBuffer "hello" -> ${r}`)
}

{
  const r = base64.decode("aGVsbG8=") as ByteBuffer
  console.assert(r instanceof ByteBuffer, `decode result is ByteBuffer: ${typeof r}`)
  const arr = r.toArray()
...

{
  ...
  console.assert(typeof r === "string", `decode utf8 mode returns string: ${typeof r}`)
  console.assert(r === "hello", `decode utf8 -> "${r}"`)
}

{
  const r = base64.decode("aGVsbG8") as ByteBuffer
  console.assert(r instanceof ByteBuffer, `decode no-pad returns ByteBuffer`)
  const arr = r.toArray()
  const expected = [104, 101, 108, 108, 111]
  console.assert(
    arr.length === expected.length && arr.every((v: number, i: number) => v === expected[i]),
...
{
  const r = base64.decode("aGVsbG8", { mode: "utf8" })
  console.assert(r === "hello", `decode no-pad utf8 -> "${r}"`)
}

{
  ...
  console.assert(threw, "decode invalid chars throws")
}

{
  const encoded = base64.encode("hello")
  const decoded = base64.decode(encoded, { mode: "utf8" })
  console.assert(decoded === "hello", `roundtrip utf8 -> "${decoded}"`)
}
