async function testHeadersGet() {
  ...
  assert(h.get("X-Test") === "value", "Headers: get")
  assert(h.get("missing") === null, "Headers: missing returns null")
}

async function testHeadersSet() {
  const h = new Headers()
  h.set("X-Test", "first")
  assert(h.get("X-Test") === "first", "Headers: set")
  h.set("X-Test", "second")
  assert(h.get("X-Test") === "second", "Headers: set overwrites")
}
...
async function testHeadersAppend() {
  ...
  h.append("X-Test", "one")
  h.append("X-Test", "two")
  assert(h.has("X-Test"), "Headers: has after append")
  assert(h.get("X-Test") === "one", "Headers: get returns first after append")
}

async function testHeadersHas() {
  const h = new Headers({ "X-Test": "value" })
  assert(h.has("X-Test"), "Headers: has existing")
  assert(!h.has("missing"), "Headers: has missing")
}

async function testHeadersDelete() {
  const h = new Headers({ "X-Test": "value" })
  h.delete("X-Test")
  assert(!h.has("X-Test"), "Headers: delete removes")
...
async function testHeadersCaseInsensitive() {
  ...
  assert(h.get("content-type") === "text/html", "Headers: case insensitive")
}
...
async function testHeadersKeysValuesEntries() {
  ...
  const v = h.values()
  const e = h.entries()
  assert(k.length === 2, "Headers: keys count")
  assert(v.length === 2, "Headers: values count")
  assert(e.length === 2, "Headers: entries count")
}

async function testAbortController() {
  const ctrl = new AbortController()
  assert(!ctrl.signal.aborted, "AbortController: initially not aborted")
  ctrl.abort()
  assert(ctrl.signal.aborted, "AbortController: aborted after abort()")
}

async function testAbortReason() {
  const ctrl = new AbortController()
  ctrl.abort()
  assert(ctrl.signal.reason === "The operation was aborted", "AbortController: default reason")
}
...
async function testAbortCustomReason() {
  ...
  try {
    ...
    assert(false, "Abort custom reason: should have thrown")
  } catch (e) {
    assert((e as Error).name === "AbortError", "Abort custom reason: must be AbortError")
...
async function testAbortSignalTimeout() {
  ...
  assert(!signal.aborted, "AbortSignal.timeout: initially not aborted")

  await new Promise<void>((resolve) => setTimeout(resolve, 50))

  assert(signal.aborted, "AbortSignal.timeout: aborted after timeout")
  assert(signal.reason === "The operation timed out", "AbortSignal.timeout: reason is timeout message")
...
async function testFetchTimeout() {
  try {
    ...
    assert(false, "Fetch timeout: should have thrown")
  } catch (e) {
    assert((e as Error).name === "AbortError", "Fetch timeout: must be AbortError")
...
async function testFetchAbortBefore() {
  ...
  try {
    await fetch("https://httpbin.org/get", { signal: ctrl.signal })
    assert(false, "Fetch abort before: should have thrown")
  } catch (e) {
    assert((e as Error).name === "AbortError", "Fetch abort before: must be AbortError")
    assert(e instanceof Error, "Fetch abort before: instanceof Error")
...
async function testFetchPost() {
  ...
  assert(r.status === 200, "fetch POST: status 200")
  const j = await r.json()
  assert(j.data === "hello world", "fetch POST: body echoed")
}
...
async function testFetchHeaders() {
  ...
  const j = await r.json()
  assert(j.headers["X-Custom"] === "test-value", "fetch headers: custom header sent")
}

async function testResponseClone() {
  const r = await fetch("https://httpbin.org/get")
  const r2 = r.clone()
  assert(r.status === r2.status, "clone: same status")
}

async function test404() {
  const r = await fetch("https://httpbin.org/status/404")
  assert(r.status === 404, "404: correct status")
...
async function testArrayBuffer() {
  ...
  assert(buf.byteLength > 0, "arrayBuffer: has length")
}

async function testResponseUrl() {
  const r = await fetch("https://httpbin.org/get")
  assert(r.url === "https://httpbin.org/get", "Response: url matches")
}

async function testResponseStatusText() {
  const r = await fetch("https://httpbin.org/get")
  assert(typeof r.statusText === "string", "Response: statusText is string")
  assert(r.statusText.length > 0, "Response: statusText not empty")
}

async function testFetchInvalidUrl() {
  try {
    await fetch("http://[::1]:99999")
    assert(false, "Invalid URL: should have thrown")
  } catch (e) {
...
async function testFetchDnsFailure() {
  try {
    ...
    assert(false, "DNS failure: should have thrown")
  } catch (e) {
...
async function testFetchAbortDuring() {
  try {
    ...
    assert((e as Error).name === "AbortError", "Abort during fetch: must be AbortError")
